...
def verify_signature(package_name, version):
    ...
    public_key_data = next(
        (key for key in public_keys if key["keyid"] == signature_data["keyid"]), None)
    ...
    verification_data = f"{package_name}@{version}:{version_data['dist']['integrity']}".encode(
        'utf-8')
...
if __name__ == "__main__":
    ...
    print(
        f"Signature is {'valid' if is_valid else 'invalid'} for {package_name}@{version}")
